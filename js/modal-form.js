((d, w, c) => {
         ...
         selector[index].addEventListener('click', function (e) {
            $selectOptions.forEach(contentOptions => {
               if (e.currentTarget.dataset.select == contentOptions.dataset.content) {
                  openOptions(e)
                  if (e.currentTarget === selector[2] && !selector[1].querySelector('.modal-form__item')) return
   ...
   function disabledSelect(e) {
      ...
      if (childTwo.querySelector('.disabled')) {
         ...
         if (!disabled) return
         disabled.classList.remove('disabled')
      } else if (childTwo.querySelector('.modal-form__placeholder') && childThree.querySelector('.modal-form__placeholder')) {
         let arrowThree = childThree.querySelector('.arrow-down')
         let selectThree = childThree.querySelector('.modal-form__select')
         arrowThree.classList.remove('blue')
         selectThree.classList.add('disabled')
      }
   }
